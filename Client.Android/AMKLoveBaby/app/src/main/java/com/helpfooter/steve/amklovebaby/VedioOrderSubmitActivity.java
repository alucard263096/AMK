...
public class VedioOrderSubmitActivity extends Activity implements View.OnClickListener,IWebLoaderCallBack {
    ...
    public void CallBack(ArrayList<AbstractObj> lstObjs) {
        ResultObj res=(ResultObj)lstObjs.get(0);

        switch (res.getId()){
            case -103:
                Toast.makeText(this, "您选择的时间已经被使用", Toast.LENGTH_LONG).show();
                return;
            case -101:
                Toast.makeText(this, "你选择的医生无效", Toast.LENGTH_LONG).show();
                return;
            case -106:
                Toast.makeText(this, "医生没有启用视频会诊功能", Toast.LENGTH_LONG).show();
                return;
            case -102:
                Toast.makeText(this, "您的会员身份无效", Toast.LENGTH_LONG).show();
                return;
            case 0:
                Toast.makeText(this, "成功创建订单，ID是"+res.getRet(), Toast.LENGTH_LONG).show();
                return;
            default:
                Toast.makeText(this, "其它错误", Toast.LENGTH_LONG).show();
                return;
        }

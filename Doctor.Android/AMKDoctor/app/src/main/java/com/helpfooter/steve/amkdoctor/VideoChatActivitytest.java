...
public class VideoChatActivitytest extends Activity implements AnyChatBaseEvent,IWebLoaderCallBack {
	...
	private SurfaceView mOtherView1;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.video_activitytest);

		Intent intent = getIntent();

		doctor_id=StaticVar.Doctor.getId();
...
//			}
	...
	private void InitLayout() {
		...
		mOtherView1 = (SurfaceView) findViewById(R.id.surface_remote1);
		...
		if (AnyChatCoreSDK
			...
			try {
				anychatSDK.mVideoHelper.SetVideoUser(anychatSDK.mVideoHelper.bindVideo(mOtherView1.getHolder()), userID1);//
			}
			catch (Exception ex)
			{
				throw ex;
	...
	Runnable runnable = new Runnable() {
		...
		public void run() {
			try {
				...
				{
					if (videoBitrate <= 0){
						Toast.makeText(VideoChatActivitytest.this, "对方视频中断了!", Toast.LENGTH_SHORT).show();
						// 重置下，如果对方退出了，有进去了的情况
				...
				if (mFirstGetAudioBitrate){
					if (audioBitrate <= 0){
						Toast.makeText(VideoChatActivitytest.this, "对方音频中断了!", Toast.LENGTH_SHORT).show();
						// 重置下，如果对方退出了，有进去了的情况
	...
	private void refreshAV() {
		anychatSDK.UserCameraControl(userID, 1);
		anychatSDK.UserSpeakControl(userID, 1);
		anychatSDK.UserCameraControl(userID1, 1);
		anychatSDK.UserSpeakControl(userID1, 1);
		anychatSDK.UserCameraControl(-1, 1);
		anychatSDK.UserSpeakControl(-1, 1);
		mBtnSpeakCtrl.setImageResource(R.drawable.speak_on);
		mBtnCameraCtrl.setImageResource(R.drawable.camera_on);
	...
	protected void onRestart() {
		...
		if (AnyChatCoreSDK
			...
			int index = anychatSDK.mVideoHelper.bindVideo(mOtherView
					.getHolder());
			int index1 = anychatSDK.mVideoHelper.bindVideo(mOtherView1
					.getHolder());
			anychatSDK.mVideoHelper.SetVideoUser(index, userID);
			anychatSDK.mVideoHelper.SetVideoUser(index1, userID1);
		}
	...
	protected void onPause() {
		...
		anychatSDK.UserCameraControl(userID1, 0);
		anychatSDK.UserSpeakControl(userID1, 0);
		anychatSDK.UserCameraControl(-1, 0);
		anychatSDK.UserSpeakControl(-1, 0);
	}
	...
	public void OnAnyChatEnterRoomMessage(int dwRoomId, int dwErrorCode) {
		...
		if(userIDArray.length>1){
			userID1=userIDArray[1];
			if(myUserId==userID1){
				Log.i("myUserId==userID1", "yes");
			}

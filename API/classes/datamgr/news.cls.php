 ...
 */  
 class NewsMgr
 {
	...
	public function getNews($id)
	{
		...
		$sql="select id,title,publish_date,news_type,category,doctor_id,summary,status,context,thumbnail,photo,".$this->dbmgr->getIsNull("upvote",5)." as upvote
		from tb_news m
	...
	public function getNewsList($lastupdate_time)
	{
		...
		$sql="select id,title,publish_date,news_type,category,doctor_id,summary,status,thumbnail,photo,".$this->dbmgr->getIsNull("upvote",5)." as upvote
		from tb_news m
		left join tb_news_statistic ms on m.id=ms.news_id 
		where 1=1 ";
